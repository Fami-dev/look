...
local function InitializeMainScript()
    ...
    -- ... (TEMPEL SEMUA KODE FITUR UI ANDA DI SINI) ...
    -- ... (Kode untuk Auto Farm, Hatching, Movement, dll tetap sama)
...
local function ProcessKey(key)
    ...
    -- 1. Cek apakah kunci ada di daftar master GitHub
    ...
    -- 2. Cek apakah kunci sudah pernah diklaim di Supabase
    ...
    if #existingRecord > 0 then
        -- Kunci sudah ada, cek apakah milik pengguna ini
        ...
        -- Kunci belum ada, ini adalah klaim baru
...
-- ALUR EKSEKUSI UTAMA
local userKey = ArcvourKey -- DIUBAH: HANYA MENCARI ArcvourKey
...
if success then
    InitializeMainScript() -- Jika berhasil, jalankan UI utama
end
