...
do
    ...
    Tabs.Farming:Input({
        ...
        Callback = function(text)
            ...
            coinAmount = (num and num > 0) and num or 119.5846266746521 -- Default ke 119 jika tidak valid
        end
    })
                        ...
                        if winsTokenTimer >= 15 and proMgsRemote then
                            if autoFarmState.AutoWins and capturedWinsRandom then
                                ...
                                pcall(function()
                                    ...
                                    print("Auto Wins event fired with token:", capturedWinsRandom)
                                end)
                            end
                            winsTokenTimer = 0
                        end
...
-- Auto Crystal Logic with robust initialization and cooldown
local function setupAutoCrystalLogic()
    ...
    local crystalCooldown = false
    local lastCrystalTime = 0
            ...
            if magicTokenValue and player then
                ...
                magicTokenValue.Changed:Connect(function(newValue)
                    ...
                    if autoFarmState.AutoCoins and autoFarmState.AutoCrystal and capturedCrystalRandom and proMgsRemote and player then
                        if newValue == 1 and lastValue == 0 and not crystalCooldown then
                            if crystalTask then
                            ...
                            crystalTask = task.spawn(function()
                                ...
                                crystalCooldown = true
                                lastCrystalTime = currentTime
                                task.wait(0.5) -- Jeda awal setelah panggil
                                while currentTime - lastCrystalTime < 15 and autoFarmState.AutoCrystal and player do
                                    currentTime = tick()
                                    task.wait(0.1)
                                end
                                crystalCooldown = false
                            end)
                        end
                        lastValue = newValue
                    end
...
-- Auto Wins Logic with dependency on Auto Crystal
local function setupAutoWinsLogic()
    task.spawn(function()
        while autoFarmState.AutoCoins and player do
            if autoFarmState.AutoWins and capturedWinsRandom and proMgsRemote then
                local currentTime = tick()
                if currentTime - lastCrystalTime >= 15 then
                    pcall(function()
                        local winsArgs = {"ClaimRooftopWinsReward", capturedWinsRandom}
                        proMgsRemote:FireServer(unpack(winsArgs))
                        print("Auto Wins event fired with token:", capturedWinsRandom)
...
if autoFarmState.AutoWins then setupAutoWinsLogic() end

autoFarmState.AutoWinsChanged = function(value)
    autoFarmState.AutoWins = value
    if value and not autoFarmState.AutoWinsTask then
        setupAutoWinsLogic()
    elseif not value and autoFarmState.AutoWinsTask then
        task.cancel(autoFarmState.AutoWinsTask)
        autoFarmState.AutoWinsTask = nil
    end
end
