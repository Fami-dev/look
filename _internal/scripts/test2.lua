...
local function InitializeMainScript()
    ...
    local Tabs = {
        ...
        Server = Window:Tab({ Title = "Server", Icon = "server", ShowTabTitle = true })
    }
    ...
    do
        Tabs.Server:Section({ Title = "Server Hop" })

        local TeleportService = game:GetService("TeleportService")
        local Players = game:GetService("Players")
        local hopping = false
        local maxPlayerCount = 10

        local function getServerList()
            local requestUrl = "https://games.roblox.com/v1/games/" .. game.PlaceId .. "/servers/Public?sortOrder=Asc&limit=100"
            local success, result = pcall(function() return HttpService:GetAsync(requestUrl) end)
        ...
        Tabs.Server:Button({
            ...
            Callback = function()
                ...
                hopping = true
                WindUI:Notify({ Title = "Server Hop", Content = "Searching for the smallest server...", Duration = 4, Icon = "loader" })
                ...
                task.spawn(function()
                    ...
                    if bestServer then
                        WindUI:Notify({ Title = "Server Hop", Content = "Server found with " .. bestServer.playing .. " players. Teleporting...", Duration = 5, Icon = "check" })
                        TeleportService:TeleportToPlaceInstance(game.PlaceId, bestServer.id)
                    else
                        WindUI:Notify({ Title = "Server Hop", Content = "No smaller server could be found.", Duration = 5, Icon = "alert-triangle" })
                        hopping = false
                    end
                end)
            end
        ...
        Tabs.Server:Section({ Title = "Custom Hop" })
        Tabs.Server:Paragraph({ Title = "Info", Desc = "Find a server with less than a specific number of players." })

        Tabs.Server:Slider({
            Title = "Max Players in Server",
            Value = { Min = 1, Max = (player.MaxPlayers - 1), Default = 10 },
            Step = 1,
            Callback = function(value)
                maxPlayerCount = tonumber(value)
            end
        })
        ...
        Tabs.Server:Button({
            ...
            Callback = function()
                if hopping then return end
                hopping = true
                WindUI:Notify({ Title = "Server Hop", Content = "Searching for a server with < " .. maxPlayerCount .. " players...", Duration = 4, Icon = "loader" })
                ...
                task.spawn(function()
                    ...
                    if servers then
                        for _, server in ipairs(servers) do
                            if server.id ~= game.JobId and server.playing < server.maxPlayers and server.playing < maxPlayerCount and server.playing < Players.MaxPlayers then
                                targetServer = server
                                break
                            end
                        end
                    ...
                    if targetServer then
                        WindUI:Notify({ Title = "Server Hop", Content = "Server found with " .. targetServer.playing .. " players. Teleporting...", Duration = 5, Icon = "check" })
                        TeleportService:TeleportToPlaceInstance(game.PlaceId, targetServer.id)
                    else
                        WindUI:Notify({ Title = "Server Hop", Content = "No server matching your criteria was found.", Duration = 5, Icon = "alert-triangle" })
                        hopping = false
                    end
                end)
            end
