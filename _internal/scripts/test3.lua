...
local msgRemote = replicatedStorage:WaitForChild("Msg"):WaitForChild("RemoteEvent")
...
local function InitializeMainScript()
    local capturedRandom1, capturedRandom2, capturedWinRandom, capturedTokenRandom
    ...
    oldNamecall = hookmetamethod(game, "__namecall", function(self, ...)
        ...
        if self == proMgsRemote and method == "FireServer" and args[1] then
            ...
            if eventId == "\232\181\183\232\183\179" then
            ...
            elseif eventId == "\232\144\189\229\156\176" then
                capturedRandom2 = args[2]
            elseif eventId == "\233\162\134\229\143\150\230\165\188\233\161\182wins" then
                capturedWinRandom = args[2]
            elseif eventId == "\233\162\134\229\143\150\230\165\188\233\161\182MagicToken" then
                capturedTokenRandom = args[2]
            end
        end
    ...
    local Tabs = {
        ...
        Misc = Window:Tab({ Title = "Misc", Icon = "grid", ShowTabTitle = true })
    }
                ...
                if value then
                    ...
                    task.spawn(function()
                        ...
                        while autoFarmState.AutoCoins do
                            local canProceed = (coinEventFormat == 1 and capturedRandom1 and capturedRandom2) or (coinEventFormat == 2 and capturedRandom2)
                            if canProceed then
                                 pcall(function()
                                    if coinEventFormat == 1 then
                                        proMgsRemote:FireServer("\232\181\183\232\183\179", capturedRandom1, coinAmount)
                                    elseif coinEventFormat == 2 then
                                        local args1 = {[1] = "\232\181\183\232\183\179", [3] = coinAmount}
                                        proMgsRemote:FireServer(unpack(args1))
                                    end
                                    proMgsRemote:FireServer("\232\144\189\229\156\176", capturedRandom2)
                                end)
                            else
                                ...
                                if autoFarmState.AutoWins or autoFarmState.AutoMagicToken then
                                    if autoFarmState.TeleportMode then
                                        if player.Character and player.Character.HumanoidRootPart and towerCoordinates[selectedMap] then
                                            ...
                                            if autoFarmState.AutoWins and capturedWinRandom then proMgsRemote:FireServer("\233\162\134\229\143\150\230\165\188\233\161\182wins", capturedWinRandom) end
                                            if autoFarmState.AutoMagicToken and capturedTokenRandom then proMgsRemote:FireServer("\233\162\134\229\143\150\230\165\188\233\161\182MagicToken", capturedTokenRandom) end
    
                                            task.wait(0.5)
                                            rootPart.CFrame = CFrame.new(destinationCoords.X, destinationCoords.Y, 0)
                                            task.wait(0.1)
    ...
    do
        ...
        Tabs.Misc:Toggle({
            ...
            Callback = function(value)
                ...
                if value then
                    task.spawn(function()
                        while autoFarmState.AutoClaimGift do
                            for i = 1, 12 do
                                pcall(function()
                                    msgRemote:FireServer("\233\162\134\229\143\150\229\156\168\231\186\191\229\165\150\229\138\177", i)
                                end)
                                task.wait(0.1)
                            end
                            task.wait(5)
        ...
        Tabs.Misc:Section({ Title = "Auto Spin" })
        Tabs.Misc:Toggle({
            Title = "Auto Spin Daily",
            Value = false,
            Callback = function(value)
                autoFarmState.AutoSpin = value
                if value then
                    task.spawn(function()
                        while autoFarmState.AutoSpin do
                            pcall(function()
                                local spinButton = player.PlayerGui.ScreenGui:FindFirstChild("\230\175\143\230\151\165\232\189\172\231\155\152", true)
                                if spinButton and spinButton:FindFirstChild("Frame") and spinButton.Frame:FindFirstChild("BtnFrame") and spinButton.Frame.BtnFrame:FindFirstChild("BtnSpin") and spinButton.Frame.BtnFrame.BtnSpin:FindFirstChild("Button") then
                                    spinButton.Frame.BtnFrame.BtnSpin.Button.MouseButton1Click:Fire()
                                end
                            end)
                            task.wait(10)
                        end
